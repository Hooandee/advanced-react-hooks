...
- Is not recomended to use default values in useContext because the value should come from the provider, and setting a default will make the state to be out of sync.
- Keep the context on the tree that belongs, not globally
- You can use contexts for caching
- Maybe just lifting up the state is enough instead of using context
