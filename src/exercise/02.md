...
- useCallBack are a shorthand for useMemo in functions
- Used when a function is triggered within an useEffect and we don't want it to change its reference.
- Functions comming from dependencies modules will never change, that's the reason are not needed in the dependency array of an use effect.
- Memoized functions could be garbage collected (if user scrolls) and called again, important if they need to make HTTP calls.
