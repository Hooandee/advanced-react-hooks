...
- The function passed to the useReducer needs the state and the action
- The second value retrieved from useReducer is the dispatcher, it accepts a value and automatically passes the current state to the reducer function
- We can use a third function for the useReducer for lazy initialization. It will take the second argument as argument, and the return value will be used as initial value for the reducer
